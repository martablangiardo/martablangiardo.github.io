...
From these preliminary exploratory analyses is evident that there are large differences in life expectancy at birth between the least and most deprived areas.
...
The results of this regression model show that the deprivation score and sex are significant predictors of life expectancy. In particular:

* `IMDscore` (i.e. $\beta_1$ in the equation) represents the average difference in life expectancy comparing units that differ by 1 unit on IMD score 

* `SexMale` (i.e. $\beta_2$ in the equation) estimates the difference in life expectancy between males and females, given a particular value for IMD score. 

* The intercept when `Sex = Female` equals 71.504 and the intercept when `Sex = Male` equals 71.504-0.540=70.964
...
* Plot the marginal posterior distributions of $\beta_0$, $\beta_1$ and $\beta_2$
