---
title: "Practical 3 - hospital admissions for respiratory conditions in Greater Glasgow and Clyde (2007-2011)"
...
setwd("~/Dropbox/Books/INLABook/ShortCourse/VIBASS/Practical3")
RESP_DATA <- read.csv("RESP_DATA.csv", header=TRUE)
```
...
\log(\lambda_{i}) & = & b_0 + v_{i} + u_{i} \\
v_{i} & \sim & N(0, \sigma_{v}^2)\\
{\bf u} & \sim & \hbox{ICAR}({\bf W}, \sigma_{u}^2)\\
b_0 & \sim & \text{Uniform}(-\infty, +\infty) \\
1/\sigma_{v}^2 & \sim & \text{Gamma}(1,0.001) \\
1/\sigma_{u}^2 & \sim & \text{Gamma}(0.5,0.005) \\
\end{eqnarray}
\]
...
\begin{eqnarray}
O_{i}|\lambda_{i}  & \sim & \text{Poisson}(\lambda_{i}E_{i} )  \\
\log(\lambda_{i}) & = & b_0 + v_{i} + u_{i} + \psi_t + \gamma_t \\
v_{i} & \sim & N(0, \sigma_{v}^2)\\
{\bf u} & \sim & \hbox{ICAR}({\bf W}, \sigma_{u}^2)\\
\psi_t &\sim & N(0, \sigma_{\psi}^2) \\
\gamma_t & \sim & RW1(\sigma_{\gamma}^2) \\
b_0 & \sim & \text{Uniform}(-\infty, +\infty) \\
1/\sigma_{v}^2 & \sim & \text{Gamma}(1,0.001) \\
1/\sigma_{u}^2 & \sim & \text{Gamma}(0.5,0.005) \\
1/\sigma_{\gamma}^2 & \sim & \text{Gamma}(1,0.001) \\
1/\sigma_{\psi}^2 & \sim & \text{Gamma}(0.5,0.005) \\
\end{eqnarray}
\]
...
\begin{eqnarray}
O_{i}|\lambda_{i}  & \sim & \text{Poisson}(\lambda_{i}E_{i} )  \\
\log(\lambda_{i}) & = & b_0 + v_{i} + u_{i} + \psi_t + \gamma_t + \delta{it}\\
v_{i} & \sim & N(0, \sigma_{v}^2)\\
{\bf u} & \sim & \hbox{ICAR}({\bf W}, \sigma_{u}^2)\\
\psi_t &\sim & N(0, \sigma_{\psi}^2) \\
\gamma_t & \sim & RW1(\sigma_{\gamma}^2) \\
\delta_{it} & \sim & N(0, \sigma_{\zeta}^2) \\
b_0 & \sim & \text{Uniform}(-\infty, +\infty) \\
1/\sigma_{v}^2 & \sim & \text{Gamma}(1,0.001) \\
1/\sigma_{u}^2 & \sim & \text{Gamma}(0.5,0.005) \\
1/\sigma_{\gamma}^2 & \sim & \text{Gamma}(1,0.001) \\
1/\sigma_{\psi}^2 & \sim & \text{Gamma}(0.5,0.005) \\
1/\sigma_{\delta}^2 & \sim & \text{Gamma}(1,0.001) 
\end{eqnarray}
\]
