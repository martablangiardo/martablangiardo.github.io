...
* Extract now the posterior distribution of the **fixed effects** 
```{r}
output$summary.fixed
```
and we note the significative and negative effect of elevation (even if small). 
...
output$dic$dic
```
Just notice that the computation of the DIC here is just for exercise. It has not meaning here, because we are not comparing competitive models.
