---
title: "Lab 5 - A space-time model with `R_inla`"
...
The dataset used for this lab is included in the package `spTimer` by @Bakar2015. The data available are as follows:
...
* Now, we print the first rows of the data 
...
We use the function `glimpse()` from the R package `dplyr` to see every column in a data frame. It's a little like `str()` applied to a data frame but it tries to show you as much data as possible
...
A required parameter in the latter function is `max.edge`, which determines the largest allowed triangle length. If the value supplied to this argument is a scalar, it controls the triangle edge lengths in the inner domain; if a vector of two values is supplied, it controls the edge lengths in the inner domain and in the outer extension respectively. Notice that the value (or values) passed to the `max.edge` function must be on the same scale unit as the coordinates. 

* Several studies, tutorials and group discussions have suggested the `max.edge` value to be between 1/3 to 1/10 times smaller than the spatial range (see for example: https://haakonbakkagit.github.io/btopic104.html or this well done recent post https://rpubs.com/jafet089/886687). Here we we make assumption that the range is about 1/3 of the study area and thus specify the `max.edge` value to be 1/5 of the spatial range 

```{r}
#max.edge = diff(range(NYdata[,2]))/15
max.edge = 0.5
```
...
coords.cv = unique(DataFit[c("s.index","Longitude","Latitude")])
...
mesh = inla.mesh.2d(loc=coords.cv,
                    max.edge = c(1,5)*max.edge,
# - use 5 times max.edge in the outer extension/offset/boundary
                    cutoff = max.edge/5)
```

We use the `gg()` function from the `inlabru` package to plot the mesh:
```{r}
ggplot() +
  gg(mesh) +
