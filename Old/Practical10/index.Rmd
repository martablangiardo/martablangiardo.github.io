---
title: "Practical 10 - Spatio-temporal estimation and prediction of ozone concentrations in New York"
...
8-hour average ground level ozone ($O_3$, ppb) concentrations for the period July 1 and August 31 in 2006 (62 days), measured at 28 monitoring sites in the state of New York, USA. 
...
We use `y8hrmax` concentrations as outcome and `xmaxtemp`, `xwdsp`, and `xrh` as predictors. For computational reasons in this lab we will work with a subset of the data.
...
```{r eval=FALSE, echo=TRUE, message=FALSE, fig.cap = "Map of the monitoring stations in New York State"}
...
```{r eval=TRUE, echo=TRUE, include=FALSE, message=FALSE}
nysptime = nysptime %>%
...
```{r eval=FALSE, echo=TRUE, fig.cap = "Daily ozone levels for 28 monitoring stations, Jul 1– Aug 31, 2006"}
...
```{r eval=FALSE, echo=TRUE, message=FALSE, fig.cap = "Histgram of ozone concentrations"}
...
```{r eval=FALSE, echo=TRUE, message=FALSE, fig.cap = "Figure 4. Plot of the correlation between variables in analysis", fig.height=6, fig.width = 11}
library(GGally)
...
* In this lab, we want also study the predictive capability of the model. To do so, we prepare two different datasets: the first, containing data for 8 stations, will be used for model validation, the second, with the data referring to the remaining stations, will be used for model training:
```{r}
# validation sites
...
DataFit = nysptime %>% 
  filter(!(s.index %in% s))
dim(DataFit)
# Dim = 600 (20stations*30days) * 15

# Validation data
DataValPred =  nysptime %>% 
  filter(s.index %in% s)
dim(DataValPred)
# dim = 240 (8stations*30days) * 15
```
...
Now, we implement the spatio-temporal model for ozone concentrations. Let
...
+ $\boldsymbol{\eta}_t=(\eta(\mathbf{s}_1,t), \dots, \eta(\mathbf{s}_n,t))'$ are the spatio-temporal random effects, assumed to follow $N(\mathbf{0}, \Sigma_{\eta})$  independently in time, where $\Sigma_{\eta}=\sigma_{\eta}^{2} S_{\eta}$, here $\sigma_{\eta}^2$ is the site invariant spatial variance  and $S_{\eta}$ is the spatial correlation matrix. We assume that the latent spatio-temporal process changes in time with first order autoregressive (AR(1)) model.
...
The model we are going to implement is a separable model similar to the one described in @cameletti2013. 
...
* Now we plot the mesh with the station points included in the training dataset. To do so, we use the `gg()` function from the `inlabru` package
```{r eval=FALSE, echo=TRUE}
ggplot() +
  gg(mesh.cv) +
...
### 4.2 Create the index set and matrix A

* Create the index sets for the training data
```{r eval=FALSE, echo=TRUE}
n_days = length(unique(DataFit$time))
n_spatial.cv = mesh.cv$n 

s_index.cv = inla.spde.make.index(
  name = "spatial.field",
...
```{r eval=FALSE, echo=TRUE}
coordinates.alldays.cv = DataFit[c("Longitude", "Latitude")] %>% as.matrix()

Ae = inla.spde.make.A(mesh = mesh.cv,
...
```{r eval=FALSE, echo=TRUE}
nrow(DataFit) 
length(s_index.cv$spatial.field) 
```

### 4.3. Define the stacks for the estimation and the predictions at the hold-out validation sites

* Define the stack for the estimation part

```{r eval=FALSE, echo=FALSE, include=FALSE}
stk_est = inla.stack(
  data = list(y = DataFit$sqrty8hrmax),
  A = list(Ae, 1),
  effects=list(c(s_index.cv,
        list(Intercept=1)),
        list(DataFit[,10:12])),
  tag="est")
```
...
(i) We now construct a stack for the prediction locations. To do so, we construct the matrix `Ap` from the prediction locations to the mesh nodes. We specify the prediction locations as `coords.val` 

```{r eval=FALSE, echo=TRUE}
coords.val = DataValPred[c("Longitude", "Latitude")]

groupp = DataValPred[, 14]

# Create A matrix for grid prediction 
Ap = inla.spde.make.A(
  mesh =  mesh.cv,
  loc = as.matrix(coords.val),
 group = groupp, 
  n.group = n_days)
dim(Ap)
```

(ii) Finally, we create the stack for the point predictions and the joint stack
```{r eval=FALSE, echo=TRUE}
stk_pred = inla.stack(
  data = list(y = NA),
  A = list(Ap, 1),
  effects=list(c(s_index.cv, #Spatial field 
          list(Intercept=1)),
          list(DataValPred[,10:12])),
          tag="pred")
...
### 4.4 Run INLA for the cross-validation excercise
...
fit.cv = inla(
  formula.cv,
  data = inla.stack.data(stk, spde = spde.cv),
  family = "gaussian",
...
* We will explore the output and discuss the results in next sections 5-7, while here we complete the cross validation exercise by plotting together the observed (red) and predicted time series (posterior mean in black); the gray ribbon describes the posterior 95% interval to inspect the predictive capability of our model:

```{r eval=FALSE, echo=TRUE}
index.pred.cv = inla.stack.index(stk, "pred")$data

## add to modmat the prediction and their sd
DataValPred$yPostMean = fit.cv$summary.fitted.values[index.pred.cv, "mean"]
DataValPred$ySD <- fit.cv$summary.fitted.values[index.pred.cv, "sd"]
DataValPred$q0.025 = fit.cv$summary.fitted.values[index.pred.cv, "0.025quant"]
DataValPred$q0.975 = fit.cv$summary.fitted.values[index.pred.cv, "0.975quant"]

## plot with the original data
plot.cv = as.data.frame(DataValPred) %>%
  ggplot() + 
  geom_line(aes(time, sqrty8hrmax, group = s.index), color = "red") +
  geom_line(aes(time, yPostMean, group = s.index)) +
...
We now repeat all the same process to implement the model, but using the entire data set, i.e. `nysptime` and obtaining predictions on the entire domain.

### 5.1. Create the mesh and the SPDE model
...
* Given the mesh, it is now possible to create the SPDE model using the `inla.spde2.matern` function or `inla.spde2.pcmatern`. Here we use PC priors for the model. Create the SPDE model as we did before and call it `spde`
...
### 5.2 Create the index set and matrix A

* Create the index sets. Call it `s_index`
```{r eval=FALSE, echo=FALSE, include=FALSE}
...
* Create the project matrix using the function `inla.spde.make.A` function. Remember from the previous section that this function takes as arguments the mesh, the measurement locations `loc`, the measurement group (here day), the number of groups. Call the projector matrix for the estimation `A_est`
```{r eval=FALSE, echo=FALSE, include=FALSE}
...
* Check the dimension of the projector matrix
```{r eval=FALSE, echo=FALSE}
dim(A_est) 
...
### 5.3. Define the stacks for estimation and prediction

* Define the stack for the estimation part. Call it `stack_est`
...
(iii) For the sake of simplicity we consider only the first time point:
```{r eval=FALSE, echo=TRUE}
...
(v) Finally, we create the stack for the predictions and the joint stack as we did before. Call the stack for the predictions as `stack_pred` and the full stack as `stack`
```{r eval=FALSE, echo=FALSE, include=FALSE}
...
## 6.  Extract and analyse the posterior summaries 
...
## 7. Map of the predictions at grid level

* We extract the indices to the prediction nodes and then extract the posterior mean of the response at day 1. To do so, we use `inla.stack.index()` function to get the indices of stack that correspond to `tag = "pred"`
...
* Finally we present the map of the smooth ozone concentration posterior mean (on the square root scale) and of the prediction standard error:
```{r eval=FALSE, echo=TRUE}
# posterior mean
ggplot() + 
  gg(post.mean.df, aes(Longitude, Latitude, fill = post.mean.pred))  +
  ggtitle("Posterior mean at 2006-07-01") +
  scale_fill_viridis() +
  coord_equal()+
  geom_point(data = coords, aes(Longitude, Latitude))
...
ggplot() + 
  gg(post.mean.df, aes(Longitude, Latitude, fill = post.sd.pred))  +
  ggtitle("Posterior sd at 2006-07-01") +
  scale_fill_viridis() +
  coord_equal()+
  geom_point(data = coords, aes(Longitude, Latitude))
```
