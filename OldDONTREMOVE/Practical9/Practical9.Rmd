...
```{r echo=FALSE, eval=FALSE, fig.width=5.5, fig.height=4}
ggplot() + 
...
7. Compute the standardized morbidity ratios (SMR)
...
```{r mapSMR, eval=TRUE, echo=FALSE, include=FALSE, fig.cap="Map of the average SMRs over the period 2007-2011"}
...
9. Fit the hierarchical Poisson log-linear model in `INLA`. Call the output as `sBYM.model`. Remember to create an ID for the areas, i.e. index vector, and use a `bym2` prior for the spatial random effect specifying the PC priors [@Simpson2017] seen in Session 2.2. Monitor also the Watanabe-Akaike information criterion (WAIC) including `control.compute=list(waic=TRUE)`
...
* Remember to join `sf` object and data frame with the posterior estimates (call the object `map_RR_PP`)
```{r eval=TRUE, echo=FALSE, include=FALSE}
map_RR_PP = left_join(GGHB, resRR_PP, by = c("SP_ID" = "SP_ID"))
```


* Produce the maps of the posterior mean of the `residual RRs` and the posterior probabilities `PP` using `ggplot2` package. Call the two plots as `p1` and `p2`

```{r mapRRsp, eval=TRUE, echo=FALSE, include=FALSE, fig.width=12, fig.height=4}
...
```{r mapRRst, eval=FALSE, echo=TRUE, fig.cap="Spatio-temporal model: Map of the residual RRs and posterior probabilities", fig.width=12, fig.height=10}
...
19. Create the posterior mean for the spatial and temporal effects as we did in the previous section, and compare with the ST model results without interaction
```{r eval=TRUE} 
#Spatial Relative risks
RR_stIntI.BYM = c()
...
```{r mapRRstIntI, eval=FALSE, echo=TRUE, include=TRUE, fig.cap="Spatio-temporal model: Map of the residual RRs and posterior probabilities", fig.width=12, fig.height=10}
...
```{r eval=FALSE, echo=TRUE, fig.width=20, fig.height=10}
