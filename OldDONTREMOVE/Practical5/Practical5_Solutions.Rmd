...
head(reading)
```
Note that $ID$ is the identifier for each student


* We now see an alternative and more elegant way to print the first five rows of the data, formatting these in a table. RMarkdown by default displays data frames and matrices as they would be in the R terminal. To display a table with additional formatting, we can use the `knitr::kable` function:
...
ANSWER:
We see a clear improvement in reading scores, even so is evident that there is a between-child variation.
Clearly the observations within an individual are not independent, thus we need to reflect this in our model. 
...
ANSWER:
The reading scores increase about five points a year
...
ANSWER:
The posterior mean of $\sigma_{\alpha}$ is about 5, and is similar to that obtained for $\beta_1$, thus the variation between students at a given point in time is comparable to the amount they might be expected to improve over one year. Also, because $\sigma_{\alpha}$ is bigger than zero, we cannot claim the students are all the same.
...
INTERPRETATION: the mean is zero as would be expected given the specification of the model (we assumed an iid Gaussian model centered around 0). From the quartiles, we see that about half the students have mean scores within about [-3, 3] of the overall mean. One student is 14 points above the mean. 
...
Answer:
We see that for the first individual, who has achieved relatively weak reading scores at the first two time-points, the predictive distribution is shifted below the overall mean of 40.8 expected at age 10.5 years. In contrast, we see that the predictive distribution for the individual without known response information is centered on this expected mean. 
For the former distribution, we have some information about the student so the prediction is better (has a more concentrated distribution). Instead, we have no information in the latter case, so the predictive distribution has greater spread.
