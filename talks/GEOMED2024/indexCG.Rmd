...
.image-6[ <img src="img/COVID_Policy.png" alt="" style="width:450px;"/> ]
...
$$h(\mu_{it}) = {\color{red}{\alpha_0 + \alpha_1 w_{i}}} +
...
&\phi_{0} z_{t}^{1} (t - t_{0}^{1}) + \phi_{j} z_{t}^{1} (t - t_{0}^{1}) w_{j_i} + \\
&{\color{red}{\delta_{0}^{\star} z_{t}^{2} + \delta_{j}^{\star} z_{t}^{2} w_{j_i}}} + \\
&\delta_{0} z_{t}^{2} (t - t_{0}^{2}) + \delta_{j} z_{t}^{2} (t - t_{0}^{2}) w_{j_i} + \\
...
- Caveat: it cannot be framed in a traditional causality perspective entirely, but it allows to evaluate the effect of intervention in a regression-type model

`r vspace("30px")`


- Still some assumptions are needed
