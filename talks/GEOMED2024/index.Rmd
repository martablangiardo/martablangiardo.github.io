...
.image-1[ <img src="img/Vaccination.jpg" alt="" style="width:400px;"/> ]
...
.image-2[ <img src="img/Brexit.png" alt="" style="width:400px;"/> ]
...
.image-4[ <img src="img/LEZ.jpg" alt="" style="width:400px;"/> ]
...
.image-5[ <img src="img/SmokingBan.jpg" alt="" style="width:400px;"/> ]
...
.image-6[ <img src="img/SmokingBan2.png" alt="" style="width:400px;"/> ]
...
.image-7[ <img src="img/COVID_Policy.png" alt="" style="width:400px;"/> ]
...
.image-8[ <img src="img/Speed_limit.png" alt="" style="width:400px;"/> ]

--

`r vspace("500px")`
 
- Evaluating the effects of policy interventions is of crucial importance.

`r vspace("40px")`

- Robust statistical methods needed.
...
Empirical intervention study used to estimate the causal impact of an intervention on its target population without random assignment.]
...
- Quasi-experimental design in a longitudinal framework.

`r vspace("20px")`

- Adding dependency: hierarchical framework as a general framework.
...
- Future directions: Including Negative Outcome Controls.
...
A time series of a particular outcome of interest is ``interrupted'' by an intervention at
a known point in time.

Particularly useful for ``natural experiments'' in real
word settings.
]
...
Other methods under this umbrella: segmented regression, change-in-change, regression discontinuity...
...
For each unit $i=1,\ldots,I$ (areas or individuals) and time $t=1,\ldots, T\geq \text{2}$,
the data comprise: 
...
- an indicator variable $z_{t}=1$ if the intervention is being applied at
...
$$h(\mu_{it}) =
\underbrace{\underbrace{\alpha_0 + \alpha_1 w_i +
\sum_{k=1}^K \beta_k X_{itk} + \delta_0 z_{t} + \delta_1 w_i z_t+
\gamma_i }_{\mbox{DID}}+ \lambda_t}_{\mbox{ITS}}$$
...
- $\delta_1$ identifies the causal effect of the intervention. Additional time dependent causal effects can be obtained through $\lambda_t$ if appropriate.
...
.pull-left[
`r include_fig("NOincinerator.png", width="75%")`
]

.pull-right[
`r include_fig("Incinerators.png",width="75%")`
]
...
$y_{it} \sim Bin(\mu_{it},n_{it} )$ for  infant mortality;

$y_{iqt} \sim \mathcal{N}(\mu_{it}, \sigma^2)$ for sex ratio.  


On the linear predictor

$$h(\mu_{it}) = \alpha_0 + \alpha_1 w_{i} +
\lambda t + \psi w_i t  + \delta_0 z_{t}(t-t_0) + \delta_1 w_i z_{t}(t-t_0) + \sum_{k=1}^K \beta_k X_{ik} + \gamma_i$$
...
.pull-right[
- Linear trends on $\lambda_t$ and $\psi_t$
- On $\gamma_i$  we assume a combination of spatial and unstructured random effects
- Disconnected spatial areas

`r include_fig("DiscGraph.png")`
...
# Welfare policies and mental health
...
`r vspace("20px")`
\begin{align*}
h(\mu_{it}) &= \alpha_0 + \alpha_{j_i} w_{i} +\\
&\psi_0 t + \psi_1 w_{j_i} t +\\
&\phi_{0}z_t + \phi_{1}w_{j_i}z_t +\\
& \delta_0 z_t(t-t_0) + \delta_1 w_{j_i}z_t(t-t_0) +\\
...
plot1
```
- evidence of greater psychological distress in people from Black Caribbean backgrounds, which persists over time
...
- Age 16-64 with 
  ...
  - (iii) responded at least once to mental health questionnaire
...
- **Confounders**  (individuals) age, education level, ethnicity, relationship sta-
tus, and sex; (area level) social deprivation and ethnic diversity  
]

.pull-right[
\begin{align*}
h(\mu_{it}) &=\alpha_0 + \alpha_1 w_{it} +\\
& \psi_0 t_i +  \psi_1 w_{it} t_i +\\
&\phi_0 z_{it} + \phi_1 w_{it} z_{it} + \\
&\delta_0 z_{it}(t-t_{0i}) + \delta_1 w_{it} z_{it}(t-t_{0i})+\\
& \sum_{k=1}^K \beta_k X_{itk} + 
...
- Start of the policy is not fixed 
...
.content-box-green[**Standardised percent change**

It estimates the change in the probability of experiencing the outcome in the exposed population before and after the intervention, while adjusting for what happens in the controls during the same period.
]
...
average of the posterior distribution of the linear predictor for all exposed individuals after the intervention only

2. $\tilde{p}^\text{EB} = \mu^\text{EB} \frac{\mu^\text{CA}}{\mu^\text{CB}}$
average of the posterior distribution of the linear predictor for all exposed individuals before the intervention only 
...
# NOC (work in progress)
.content-green-box[Negative Outcome Controls (NOC)
A Negative Outcome Control is an observed outcome not related to the treatment but that is influenced
by the same confounders as the main outcome.
]

- Can be on a different scale in comparison to the outcome.
- Re-scale both elements of $\bm{y}$ so that they are defined on $(-\infty,+\infty)$.

On the augmented data
$\bm{y}^*=\left[\begin{array}{c}\bm{y}^*_0 \\ \bm{y}^*_1\end{array}\right]\sim \mbox{Normal}(\mu_{it},\sigma^2_i)$ and indicators $\bm{w}^*=\left[\begin{array}{c}\bm{w}_0 \\ \bm{w}_1\end{array}\right]$ and $\bm{z}^*=\left[\begin{array}{c}\bm{z}_0 \\ \bm{z}_1\end{array}\right]$ we can define the BHF model
\begin{align*}
\mu_{it} &= \alpha_0+\alpha_1 w^*_i+\delta_0 z^*_t+ \delta_1 z^*_t w^*_i+\gamma_i+\lambda_t+\psi_t w^*_i
\end{align*}

- Through the posterior predictive distribution we can estimate the outcome as if the intervention did not happen and bring it back to the original scale.
- The comparison with the observed outcome time series post-intervention would provide a causal interpretation.
...
.center[`r include_fig("thankyou.png", width="50%")`]
