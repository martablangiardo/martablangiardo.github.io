...
as.data.frame(DataValPred) %>%
  ggplot() + 
  geom_line(aes(time, sqrty8hrmax, group = s.index), color = "red") +
  geom_line(aes(time, yPostMean, group = s.index)) +
  geom_ribbon(aes(time, ymin = (yPostMean - 2 * ySD), ymax = (yPostMean + 2 * ySD), group = s.index), alpha = 0.5) +
  facet_wrap(~ s.index)
```
...
* Now we create a triangulation of our domain. First we establish the boundary of the domain with the function `inla.nonconvex.hull`, then we create the mesh with `inla.mesh.2d` function, as we did before, but using all the data from the New York stations (i.e. `nysptime`)
...
* Now we plot the mesh with the 28 station points. To do so, we use the `gg()` function from the `inlabru` package
```{r}
ggplot() +
  gg(mesh) +
...
* Given the mesh, it is now possible to create the SPDE model using the `inla.spde2.matern` function or `inla.spde2.pcmatern`. Here we use PC priors for the model. Create the SPDE model as we did before
...
* Create the project matrix using the function `inla.spde.make.A` function. Remember from the previous section that this function takes as arguments the mesh, the measurement locations `loc`, the measurement group (here day), the number of groups
```{r}
...
(v) Finally, we create the stack for the predictions and the joint stack as we did before 
```{r}
...
#lp_marginals = fit$marginals.linear.predictor[index.pred]

# grid for the prediction
grid_locs = expand.grid(
  lon = seq(
  min(gridnysptime$Longitude),
  max(gridnysptime$Longitude),
  length.out = 10),
  lat = seq(
  min(gridnysptime$Latitude),
  max(gridnysptime$Latitude),
  length.out = 10))

# prepare posteriors for mapping  
post.mean.df = SpatialPixelsDataFrame(SpatialPoints(grid_locs),
data = data.frame(post.mean.pred))

postsd.df = SpatialPixelsDataFrame(SpatialPoints(grid_locs),
...
# posterior mean
ggplot() + 
  gg(post.mean.df, aes(lon, lat, fill = post.mean.pred))  +
...
ggplot() + 
  gg(post.mean.df, aes(lon, lat, fill = post.sd.pred))  +
