...
bibfile=RefManageR::ReadBib("C:/Users/magb/Dropbox/AIMS_RWANDA/Session2.2/Biblio.bib",check = FALSE)
```
...
### `r rmarkdown::metadata$param$conference`
...
Under the BYM2 specification the hyperparameters $\tau_b$ and $\phi$ are modelled using .red[**Penalised Complexity** (PC) priors] `r Cite(bibfile,"Riebler2016")`:
...
- We need to define $U_1, U_2$ and $\alpha_1, \alpha_2$; following `r Citet(bibfile, "Riebler2016")`:
`r vspace("10px")`
...
- Therefore, following `r Citet(bibfile, "Riebler2016")` the priors are as follows:

    1\. Assuming $U_1=0.5/0.31$ translates into $P(\sigma_{\tau_b}>1.62) = 0.01$, using the rule of thumb in `r Citet(bibfile, "Riebler2016")`
...
library(sf)
london.gen = read_sf("C:/Users/magb/Dropbox/AIMS_RWANDA/Session2.2/LDN/LDNSuicides.shp")
london.gen$ID = seq(1,32)
```
...
load("C:/Users/magb/Dropbox/AIMS_RWANDA/Session2.2/LDN/LondonSuicides.RData")
library(INLA)
datasuicides = tibble(y=y,E=E, ID=seq(1,32))
Nareas = 32
formula = y ~ 1 + f(ID, model="bym2",graph="C:/Users/magb/Dropbox/AIMS_RWANDA/Session2.2/LDN/LDN.graph", 
        hyper=list(prec = list(
        prior = "pc.prec",
        param = c(0.5 / 0.31, 0.01)),
        phi = list(
...
- These probabilities are called .red[exceedance probabilities] and are useful to assess unusual elevation of disease risk `r Cite(bibfile,"Richardson:2004")`
...
```{r exceedence, echo=TRUE, eval=FALSE}
