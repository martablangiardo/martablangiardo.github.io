---
title: "Session 9.1: Introduction to Geostatistics"
params: 
   conference: ""
   date: ""
   short_title: "AirAware workshop"
output:
...
bibfile=RefManageR::ReadBib("~/Dropbox/TEACHING/YEAR_2024/Bayes_Spatial_2024/Material/Session9.1/Biblio.bib",check = FALSE)
```
...
- Section 6.4 of the book **Spatial and Spatio-Temporal Bayesian models with R-INLA** https://github.com/michelacameletti/Spatial-and-Spatio-Temporal-Bayesian-Models-with-R-INLA

- Chapter 12 of the book **Spatial Statistics for Data Science: Theory and Practice with R**  https://www.paulamoraga.com/book-spatial/index.html 
...
# Geostatistical data: definition

- The difference between models for .red[**geostatistical**] (or point referenced) data and spatial models for areal data is that here we treat space as continuous, not discretised (areas).

`r vspace("20px")`

- We are concerned here with spatial data structures where the
process of interest is a spatial field 
...
- To **reconstruct the spatial field** from sparse observations taken at a finite number of spatial locations.
...
- A spatial process $Z(\bm{s})$ is a .red[**Gaussian field**] (GF) if for any $n \geq 1$ and for each set of locations $(\bm{s}_{1},\ldots,\bm{s}_{n})$, the vector $(Z(\bm{s}_1),\ldots, Z(\bm{s}_n))$ follows a .red[multivariate Normal distribution] with mean $\bm{\mu}=(\mu(\bm{s}_{1}), \ldots, \mu(\bm{s}_{n}))$ and spatially structured covariance $\bm{\Sigma}$.

`r vspace("21px")`

- The  generic element of $\bm{\Sigma}$ is defined by a  **spatial covariance function** (sometimes called a kernel) $\mathcal C(\cdot,\cdot)$
...
- A spatial process is .red[stationary] if its *statistical behaviour is similar across the map*.
  (e.g. the variability of PM2.5 is comparable in different neighbourhoods)


`r vspace("20px")`

- Why it matters: it lets us describe spatial dependence using a function of **distance**,
  enabling prediction and uncertainty quantification.
...
- The spatial process is called .red[**strict (or strong) stationary**] if shifting the locations by the same vector $\bm{h}$,
  the **joint distribution** of the values does not change.
  

- A strictly stationary process repeats itself throughout the spatial domain.


- In practice, we rarely use strict stationarity directly.
  We mainly rely on **weak (second-order) stationarity**
...
.red[Weak stationarity or second order stationarity] 

- It imposes conditions only on the mean and covariance, which are translation invariant.
`r vspace("10px")`
- The spatial process is called .red[**weak (or second-order) stationary**] if
`r vspace("10px")`
  - $\bm{\mu}$  is constant, i.e., $\mu(\bm{s}_{i}) = \mu$ for each $i$ 
`r vspace("10px")`
  - the spatial covariance function depends on length and orientation of
the vector $\bm{h}$ linking two points $\bm{s}_{i}$ and $\bm{s}_{j}= \bm{s}_{i} + \bm{h}$, that is $\mbox{Cov}\left(Z(\bm{s}_{i}),Z(\bm{s}_{j})\right)=\mathcal C(\bm{s}_{i}-\bm{s}_{j}) = \mathcal C(\bm{h})$ 

  
 `r vspace("10px")`

- Interpretation: two locations have the same covariance whenever they have the same **lag** \(h\)
  (same distance, and possibly same direction).
...
- If covariance depends **only on distance** (not direction), the process is isotropic:
$$\mathcal C(\bm{s}_{i}-\bm{s}_{j}) = \mathcal C(\|\bm{h}\|)$$

(where $\| \cdot \|$ denotes the Euclidean norm, i.e. Euclidean distance)
...
- If dependence changes with direction, we have .red[anisotropy]
  (e.g., prevailing winds, mountain ranges).

---

# Intrinsic stationarity and the variogram
...
- For a choice of two locations, we assume that:
 ...
 - the variance of increments depends only on $\bm{h}$:
...
- Interpretation: larger $\gamma(\mathbf{h})$ implies that locations separated by $\mathbf{h}$ are **less similar**.

---

# Variogram

- The (semi)variogram is useful in exploratory analysis for geostatistical data, as it summarizes the strength of association as function of the distance (and in case of anisotropy, direction).
...
# Some common isotropic variogram functions [1]
...
`r include_fig("Type_Vario_new.png",width="60%",title="")`

.center[For details, see `r Citet(bibfile,"banerjee2014")`, Sections 2.1.2 - 2.1.4]
---

# Some common isotropic variogram functions [2]
