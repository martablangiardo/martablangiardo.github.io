...
bibfile=RefManageR::ReadBib("C:/Users/magb/Dropbox/Bayes_Spatial_2025/Material/Session4.1/Biblio.bib",check = FALSE)
...
- Use the WAIC as tool for model selection.
...
- In R we can perform indirect standardization using the package `SpatialEpi` (You will do it in this week homework).
...
- Assign hyperprior distribution to $a$ and $b$, for instance $$a \sim \text{Exp}(1); b\sim \text{Exp}(1)$$ 
...
library(INLA)
LipCancer <- tibble(read.csv("C:/Users/magb/Dropbox/Bayes_Spatial_2025/Material/Datasets/scotlip/scotlip.csv"))
...
library(sf)
lip.shp <- read_sf("C:/Users/magb/Dropbox/Bayes_Spatial_2025/Material/Datasets/scotlip/scotdistricts.shp")
...
**Example**: Still considering the Lip cancer dataset, we want to evaluate if the variable $x$, which represents the percentage of agriculture, farming and fishery works, is associated with the outcome. The hypothesis is that exposure to sun could be a risk factor for lip cancer.  

```{r lip3, echo=FALSE, tidy=TRUE}
library(INLA)
LipCancer <- tibble(read.csv("C:/Users/magb/Dropbox/Bayes_Spatial_2025/Material/Datasets/scotlip/scotlip.csv"))
#LipCancer <- tibble(read.csv("~/Dropbox/Lavori condivisi/2015_Book/ShortCourse/VIBASS/Datasets/scotlip/scotlip.csv"))
...
We specify a random effect Poisson model 

\begin{eqnarray*}
 y_i &\sim& \text{Poisson}(E_i \rho_i)\\
\text{log}(\rho_i) &=& b_0 + \beta_1 x_{i} + v_i\\
v_i &\sim& \text{Normal}(0, \sigma^2_v)
\end{eqnarray*}

where $x_{i}$ is the exposure to sun in each area. Note that $b_0$ , $\beta_1$, $1/\sigma^2_v$ are given independent "noninformative" priors. 
...
.pull-left[
```{r formula_lip1, echo=TRUE, eval=TRUE}
formula.reg.inla <- y ~ 1 + x + 
  f(RECORD_ID,model="iid", hyper=list(prec=list(prior="loggamma",
  	param=c(1,0.01))))

lipcancer.reg <- inla(formula.reg.inla,family="poisson",
                          data=LipCancer, E=E,
                          control.predictor=list(compute=TRUE),
                          control.compute=list(config=TRUE),
                          control.fixed=list(mean.intercept=0,prec.intercept=0.00001))

lipcancer.reg$summary.fixed
```
]

.pull-right[

Plot the random effects for the disease mapping model and for the ecological regression
```{r swt-plot, echo=TRUE, fig.height = 10, fig.width = 14, fig.show='hold',opts=list(width="75%")}
plot(lipcancer.poisson$summary.random$RECORD_ID$mean,lipcancer.reg$summary.random$RECORD_ID$mean, ylim=c(-1.5,1.5),xlim=c(-1.5,1.5))
abline(0,1)
```
]
...
```{r output_reg, echo=FALSE, eval=TRUE, fig.height = 2.5, fig.width = 4}
output2<-data.frame(RECORD_ID=output$RECORD_ID,v_DM=lipcancer.poisson$summary.random$RECORD_ID$mean,v_REG=lipcancer.reg$summary.random$RECORD_ID$mean)
out_map2 <- left_join(lip.shp,output2, by="RECORD_ID")
breaks =  c(-1.5,-1,-0.5,0.5,1,1.5)
out_map2 <- mutate(out_map2, v_DM_cat = cut(v_DM, breaks, include.lowest = TRUE), v_REG_cat = cut(v_REG, breaks, include.lowest = TRUE)) 
```


.pull-left[
###Random effect $v_i$ from disease mapping

`r include_fig("ggplot_reg-1.png",width="80%")`


```{r ggplot_reg-1, echo=FALSE, eval=FALSE, fig.height = 2.5, fig.width = 4}
library(RColorBrewer)
ggplot() + geom_sf(data = out_map2,aes(fill = v_DM_cat)) + 
          theme_bw() + scale_fill_manual(values = c("[-1.5,-1]" = 'blue', "(-1,-0.5]" = 'green',
                               "(-0.5,0.5]" = 'yellow', "(0.5,1]" = 'orange', "(1,1.5]" = "red")) + 
...
.pull-right[
###Random effect $v_i$ from ecological regression
`r vspace("-10px")`
`r include_fig("ggplot_reg-2.png",width="80%")`

```{r ggplot_reg-2, echo=FALSE, eval=FALSE, fig.height = 2.5, fig.width = 4}
ggplot() + geom_sf(data = out_map2,aes(fill = v_REG_cat)) + 
          theme_bw() + scale_fill_manual(values = c("[-1.5,-1]" = 'blue', "(-1,-0.5]" = 'green',
                               "(-0.5,0.5]" = 'yellow', "(0.5,1]" = 'orange', "(1,1.5]" = "red")) + 
...
Pop=1000
E_asthma <- exp(0.1*air_poll + 0.5*dow)
y<-c()
for(i in 1:500){
...
The assumptions are 
  ...
  2. that there is a linear relationship between air pollution and the log number of asthma attacks
...
asthma_model1 <- inla(asthma_formula1, data=data,family="poisson", 
...
asthma_model1 <- inla(asthma_formula1, data=data,family="poisson",control.predictor=list(compute=TRUE), control.compute=list(config=TRUE,waic=TRUE , return.marginals.predictor=TRUE))
```

Then to get the $y_i$ values we need to incorporate an additional step getting a posterior sample from the fitted values using 

.pull-left[
```{r eval=TRUE, include=TRUE, echo=TRUE}
post<- inla.posterior.sample(100,asthma_model1)
names(post[[1]])
post[[1]]$latent[1:3,]
```
]
...
`r vspace("-10px")`
Note that:
- `post` is a list of 100 elements and each element includes  a value from

1\. the joint posterior distribution for the hyperparameters `post$hyperpar`

2\. joint posterior distribution for the linear predictor $\bm{\eta}$ in `post$latent` (row 1 to N) 

3\. joint posterior distribution for the random effects $\bm{v}$ in `post$latent` (N +1 to 2N)
]
...
Then we sample from a Poisson where the mean is the output of the joint posterior sample (exponentiated to go back to the original scale), for the linear predictor 

```{r eval=TRUE, include=TRUE, echo=TRUE}
#Exponentiate
predlist <- do.call(cbind, lapply(post,
                    function(X) exp(X$latent[startsWith(rownames(X$latent), "Pred")])))
#Sample from the Poisson    
...
dim(pois.samples)
pois.samples[1:3,1:10]
```
]

.pull-right[
Now we plot it against the original data
```{r eval=TRUE, fig.height = 5, fig.width = 7, fig.show='hold',opts=list(width="75%")}
pred <- data.frame(asthma=c(y,apply(pois.samples,1,median)),
                   ID = c(rep("obs",500),rep("pred",500)))

require(easyGgplot2)
p1<-ggplot2.histogram(pred, xName="asthma", groupName="ID", groupColors=c('#111111','#E69F00'), densityAlpha=0.8, binwidth=1)
p1
```
]
...
\begin{align}
y_i &\sim \text{Normal}(\theta_i,\sigma_2)\\
logit(\theta_i) &= b_0 + \beta x_i\\
b_0, \beta &\sim N(0,0.001)\\
\end{align}

The assumptions are 
  1. that the data are distributed as .red[Gaussian]
  2. that there is a linear relationship between air pollution and number of asthma attacks
...
.pull-left[
- We run the model and predict observations $y^*_1,\ldots,y^*_{500}$ based on the posterior distribution of the parameters  similarly to the previous model

```{r eval=FALSE, include=TRUE, echo=TRUE}
asthma_formula2 <- y ~ x
asthma_model2 <- inla(asthma_formula2, data=data,family="Gaussian",
                      control.predictor=list(compute=TRUE),
                      control.compute=list(config=TRUE,waic=TRUE, return.marginals.predictor=TRUE))
```

```{r eval=TRUE, include=FALSE}
asthma_formula2 <- y ~ x 
asthma_model2 <- inla(asthma_formula2, data=data,family="Gaussian", control.predictor=list(compute=TRUE), 
control.compute=list(config=TRUE,waic=TRUE, return.marginals.predictor=TRUE))
post2<- inla.posterior.sample(100,asthma_model2)
#Exponentiate
predlist2 <- do.call(cbind,
                        lapply(post2,
                               function(X) exp(X$latent[startsWith(rownames(X$latent), "Pred")])))
#Sample from the Poisson    
gaussian.samples <- apply(predlist2, 2,
                          function(Z) rnorm(n = length(Z), mean = Z, sd=1/sqrt(0.2283407)))
    gaussian.samples <- as.data.frame(gaussian.samples)
...
pred2 <- data.frame(asthma=c(y,apply(gaussian.samples,1,median)),
                   ID = c(rep("obs",500),rep("pred",500)))

library(easyGgplot2)
  p2<-ggplot2.histogram(pred2, xName="asthma", groupName="ID", groupColors=c('#111111','#E69F00'), densityAlpha=0.8, binwidth=1)
p2
```
]
---
...
```{r eval=TRUE, include=TRUE, echo=TRUE}
# Model 1
Bayesian_p <- matrix(NA,500,2)

#Proportion of times the prediction is larger than the data
for(i in 1:500){
Bayesian_p[i,1] <- sum(pois.samples[i,]<y[i])/100
Bayesian_p[i,2] <- sum(gaussian.samples[i,]<y[i])/100
}
...
```{r eval=TRUE, fig.height = 10, fig.width = 14, fig.show='hold',opts=list(width="75%")}
par(mfrow=c(2,1))
hist(Bayesian_p[,1], xlim=c(0,1), ylim=c(0,80),breaks=10,col=rgb(1,0,0,0.5), xlab="", ylab="Model1", main="")
hist(Bayesian_p[,2], xlim=c(0,1),  ylim=c(0,80),breaks=10,col=rgb(0,0,1,0.5), xlab="Bayesian p-value", ylab="Model2", main="")
```
]
...
- Here model 1 seems far better than model 2
...
And now check the value of the WAIC
`r vspace("20px")`
...
```
The first model is still preferred as the WAIC  is smaller.
...
- WAIC is a useful tool for model selection, easy to calculate in INLA
