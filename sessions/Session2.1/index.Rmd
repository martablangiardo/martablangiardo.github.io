...
As a simplification let's assume that $\theta$ is discrete and can only get the values 0.1,0.4,0.8 with the following probability function, which specifies the prior probability of each possible $\theta$ value:
`r vspace("30px")`
...
```{r eval=TRUE, echo=FALSE,fig.height = 13, fig.width = 14, fig.show='hold',opts=list(width="110%")}
library(gridExtra)
library(ggplot2)
...
.pull-right[
```{r eval=TRUE, echo=FALSE,fig.height = 10, fig.width = 15, fig.show='hold',opts=list(width="110%")}
df_nn <- df[,2]*df[,4]
