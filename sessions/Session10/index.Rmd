---
title: "Session 10.1: Spatio-temporal models with INLA-SPDE"
params: 
   conference: ""
   date: ""
...
bibfile=RefManageR::ReadBib("~/Dropbox/TEACHING/YEAR_2024/Bayes_Spatial_2024/Material/Session10/Biblio.bib",check = FALSE)
```
...
- Section 7.2 of the book **Spatial and Spatio-Temporal Bayesian Models with `R-INLA`** https://github.com/michelacameletti/Spatial-and-Spatio-Temporal-Bayesian-Models-with-R-INLA
...
`r Citet(bibfile,"cameletti2013spatio")`, while the data are from `r Citet(bibfile,"moraga2019")`.
...
- The term $\omega_{it}$ refers to the **latent spatio-temporal process**, which changes in time with first order autoregressive dynamics and spatially correlated innovations:
...
- The data used to demonstrate the modelling framework are **PM2.5 concentrations** measured at several monitoring stations in Spain over the years 2015 to 2017 from the European Environment Agency. This example is taken from `r Citet(bibfile,"moraga2019")`, chapter 10.
...
.pull-right[
```{r spainmap,ref.label="spainborder", echo=F, fig.height=2.3, out.width="100%"}
```
]
...
.pull-right[
```{r spainmap2,ref.label="spainonly", echo=F, eval=FALSE, fig.height=2, out.width="100%"}
```
...
- A very powerful methodology to check the predictive capability is through  .red[**cross-validation**].
...
- In Session 10.2 we will introduce this methodology.  
