---
title: "Session 3.2: Spatio-temporal modelling for area data"
params: 
...
1\. Data
  - Disease counts $y_{ik}$ in area $i$ and stratum $k$, aggregated over a time period, $i=1,\ldots,N$, $k=1,\ldots,K$
  `r vspace("10px")`
...
\begin{align*}
\hbox{y}_t & \sim  \hbox{Poisson}(E_t \rho_t); \;\;\; t=1,...,T\\
\log \rho_t & =  ???
\end{align*}
...
$\bm \gamma_{-t}$ represent the  vector of $\gamma$s with
...
- A different parametrisation can be specified similarly to the spatial ICAR, so that 
...
- Annual rates by gender and race
...
\begin{align*}
\hbox{y}_{it} & \sim  \hbox{Poisson}(\hbox{E}_{it} \rho_{it}) \\
...
formula.mod1 <- y ~ 1 + f(county,model="bym",
                   graph=Ohio.adj, constr=TRUE) + year
```
...
Posterior means of the spatial RE ( u + v ) for the 4 different models
...
\begin{align*}
\hbox{y}_{it} & \sim  \hbox{Poisson}(\hbox{E}_{it} \rho_{it}) \\
\log\rho_{it} & =  b_0 + \theta_i + \;\gamma_t + \psi_t + \color{blue}{\delta_{it}}\\
\theta_i & =  v_i + u_i \; \hbox{(BYM = ICAR + HET)}\\
\gamma_t & \sim  \hbox{RW(1)}\\
\psi_t & \sim  \hbox{N}(0,\sigma^2_{\psi})\\
\color{blue}{\delta_{it}} & \sim  \hbox{Normal}(0, \sigma^2_{\delta})
\end{align*}
...
\begin{align*}
\log\rho_{it} & =  b_0 + \xi_i + \gamma_t + \delta_{it}\\
\xi_i & =  v_i + u_i \; \hbox{(BYM = ICAR + HET)}\\
\gamma_t & \sim  \hbox{RW(1)}\\
\psi_t & \sim  N(0,\sigma^2_{\psi})\\
\delta_{it}  \sim & \hbox{Normal}(0, \sigma^2_{\nu})
\end{align*}
...
\begin{align*}
\hbox{y}_{it} & \sim  \hbox{Poisson}(\hbox{E}_{it} \rho_{it}) \\
\log \rho_{it} & =  b_0 + v_i + u_i  + \gamma_t + \psi_t + \color{blue}{\delta_{it}}\\
...
`r include_fig("INLA_ST1_mod-1.png", width="45%")`
